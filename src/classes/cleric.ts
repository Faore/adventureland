...
class Cleric {
  ...
  public character_loop() {
    if (
      this.members_needing_healing(this.party_members).length > 1 &&
      in_attack_range(this.leader)
    ) {
      use_skill("partyheal");
    }

    var player_to_heal = this.min_by_hp(
      this.members_needing_healing(this.party_members)
    );
    ...
    if (
      this.members_needing_healing(this.party_members).length === 0 &&
      get_monster(this.leader.target)
    ) {
  ...
  public members_needing_healing(
    party_members: Array<ICharacter>
  ): Array<ICharacter> {
