...
class Wizard {
    ...
    public character_loop() {
        ...
        if(this.target) {
            ...
            if(!in_attack_range(this.target)) {
                smart_move(character.x+(this.target.x-character.x)/2, character.y+(this.target.y-character.y)/2);
            }
        }

        if(!in_attack_range(this.leader)) {
            smart_move(character.x+(this.leader.x-character.x)/2, character.y+(this.leader.y-character.y)/2);
        }
    ...
    public on_hit(hit: IHit) {
        if(hit.source == "attack" && hit.actor == this.leader.id && get_monster(hit.target)) {
