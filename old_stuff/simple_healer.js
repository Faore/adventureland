...
setInterval(function () {
  if (
    members_needing_healing(party_members, 400) > 1 &&
    in_attack_range(leader)
  ) {
    use_skill("partyheal");
  }

  var player_to_heal = min_by(
    members_needing_healing(party_members, 150),
    "hp"
  );
  ...
  if (
    members_needing_healing(party_members, 150).length === 0 &&
    get_monster(leader.target)
  ) {
